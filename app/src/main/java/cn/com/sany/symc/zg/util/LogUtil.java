...
public class LogUtil {
     ...
     *  true 为打�??，false 为关�??
     */
    public static Boolean MYLOG_SWITCH = true; // 日志文件总开�??
    ...
    private static String MYLOG_PATH_SDCARD_DIR = "PA-VTM";// 日志文件在sdcard中的路径
    private static int SDCARD_LOG_FILE_SAVE_DAYS = 0;// sd卡中日志文件的最多保存天�??
    private static int sdcard_log_file_num = 0;// 每个文件不大于2M，文件个数
    private static String MYLOGFILEName = "_log.txt";// 本类输出的日志文件名�??
        ...
        try {
            ...
            if (!TextUtils.isEmpty(path)) {
                    ...
                    File file = new File(path + File.separator + needWriteFiel
                            + MYLOGFILEName);
                    if(file.length()/(1024*1024*500) > 2){
                        ++sdcard_log_file_num;
                        file = new File(path + File.separator + needWriteFiel + "_" + sdcard_log_file_num
                                + MYLOGFILEName);
                    }
